...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("[API] Sending request to detection API:", { prompt, image_url })
      console.log("[API] Request type:", type)
      
      // 调用识别API - 使用POST请求和URL查询参数格式（按照curl示例）
      ...
      console.log("[API] Image URL:", image_url)
      console.log("[API] Image URL encoded:", encodeURIComponent(image_url))
      console.log("[API] Image URL decoded:", decodeURIComponent(encodeURIComponent(image_url)))
      ...
      if (!response.ok) {
        ...
        console.error("[API] Furniture detection API error:", response.status, errorText)
